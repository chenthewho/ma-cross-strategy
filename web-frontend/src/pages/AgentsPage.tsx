...
export default function AgentsPage() {
  ...
  return (
    <div className="space-y-4 lg:space-y-6 max-w-2xl">
      <h2 className="text-lg lg:text-xl font-semibold text-claude-text">执行端管理</h2>

      <Card className="p-4 lg:p-6 text-center">
        {api_connected ? (
          <>
            <div className="w-4 h-4 rounded-full bg-claude-success mx-auto mb-3 animate-pulse" />
            <p className="text-claude-success font-semibold text-sm lg:text-base">执行端已连接</p>
          ...
          <>
            ...
            <p className="text-claude-danger font-semibold text-sm lg:text-base">执行端未连接，交易将暂停</p>
          </>
        )}
      </Card>
      ...
      <div className="space-y-3 lg:space-y-4">
        {[
          { step: '步骤 1: 下载 Agent', desc: '下载对应平台的 Agent 可执行文件' },
          { step: '步骤 2: 配置密钥', desc: '在本地创建 config.agent.yaml，填入券商 API Key', hint: '⚠ API Key 仅存本地，永不上传云端' },
          { step: '步骤 3: 运行 Agent', desc: '运行 Agent 后，此处状态应自动更新为已连接' },
        ].map((s, i) => (
